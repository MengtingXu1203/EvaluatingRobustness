...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf, norm_type='batch', act_type='selu'):
        ...
        self.dconv3 = nn.ConvTranspose2d(ngf * 8 * 2, ngf * 4, 4, 2, 1)
        self.dconv4 = nn.ConvTranspose2d(ngf * 4 * 2, ngf * 2, 4, 2, 1)
        self.dconv5 = nn.ConvTranspose2d(ngf * 2 * 2, ngf, 4, 2, 1)
        self.dconv6 = nn.ConvTranspose2d(ngf * 2, output_nc, 4, 2, 1)
    ...
    def forward(self, input):
        ...
        # print(e4.shape) : state size is (ngf x 8) x 14 x 14
        e5 = self.norm8(self.conv5(self.leaky_relu(e4)))
        # state size is (ngf x 8) x 7 x 7
        #print(e5.shape)
        ...
        #print(d1_.shape)
        # print(d1_.shape) : state size is (ngf x 8) x 7 x 7
        d1 = torch.cat((d1_, e5), 1)
        d2_ = self.norm8(self.dconv2(self.act(d1)))
        # print(d2_.shape) : state size is (ngf x 8) x 14 x 14
